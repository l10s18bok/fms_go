...
> **Note**: UDP/ANY 선택 시 TCP Flags 옵션 영역이 표시되지만, 모든 위젯이 **비활성화(회색)** 상태로 표시됩니다. 이는 UI 일관성을 위한 것입니다.

### 7.4 프로토콜별 필드 활성화/비활성화

| 프로토콜 | 옵션 영역 | 옵션 상태 | 포트 필드 |
|----------|-----------|-----------|-----------|
| TCP | TCP Flags | 활성화 | 활성화 |
| UDP | TCP Flags | **비활성화** | 활성화 |
| ICMP | ICMP Options | 활성화 | **비활성화 (N/A)** |
| ANY | TCP Flags | **비활성화** | 활성화 |

- **ICMP**: 포트 개념이 없으므로 포트 필드가 비활성화되고 "N/A"로 표시됩니다.
- **UDP/ANY**: 옵션 영역이 비활성화 상태로 표시되어 클릭해도 동작하지 않습니다.

### 7.5 도움말 버튼

각 옵션 영역에 도움말 버튼(?)을 제공하여 사용자가 옵션에 대한 설명을 확인할 수 있습니다.
...
- **[?] 버튼 클릭**: 모달 팝업으로 해당 옵션에 대한 상세 설명 표시
...
| `internal/ui/component/rule_form.go` | 동적 옵션 필드 추가, 도움말 버튼 추가 |
| `internal/ui/component/rule_row.go` | 옵션 컬럼 추가 |
| `internal/ui/component/rule_list.go` | 헤더 컬럼 추가 |
| `internal/ui/component/help_texts.go` | 도움말 텍스트 및 공통 팝업 함수 (신규) |
