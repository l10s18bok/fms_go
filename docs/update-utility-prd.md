...
현재 FMS(Firewall Management System)는 Agent 서버를 통해 방화벽 장비에 **방화벽 룰(규칙)**을 배포하는 기능을 제공합니다. 그러나 smartfw/smartway **패키지 자체의 업데이트**는 별도의 수동 작업이 필요합니다.

본 문서는 smartfw/smartway 패키지을 원격으로 업데이트할 수 있는 유틸리티의 요구사항을 정의합니다.

### 1.2 목적

- smartfw/smartway 패키지의 원격 업데이트 자동화
...
|------|----------------------|------------------|
| 대상 | 방화벽 규칙 (방화벽 룰) | smartfw/smartway 패키지 |
...
설정된 주기(예: 5분, 10분, 30분)마다 등록된 모든 장비의 패키지 버전 정보 수집:
...
**참고:** 패키지 종류는 고정되지 않으며, 장비에 설치된 패키지에 따라 동적으로 반환됩니다.
...
│   ├── 규칙 빌더 (RuleBuilder)
│   └── NAT 빌더 (NATBuilder)
├── 패키지 관리 (ProgramTab) ← 신규
│   ├── 패키지 목록 테이블 (체크박스 다중 선택)
│   └── 버튼: 찾기, 삭제, 추가/수정
├── 장비 관리 (DeviceTab)
...
- DeviceTab → ProgramTab: 장비에 업데이트할 패키지 목록 조회 (신규)

---

### 3.2 최종 권장안: 통합 UI (작업 모드 전환)

기존 장비관리 탭을 확장하여 "방화벽 룰 배포"와 "패키지 업데이트" 모드를 라디오 버튼으로 전환하는 방식입니다.

**선정 이유:**

1. **기존 UI 구조 유지**: 사용자 학습 곡선 최소화
2. **장비 정보 일원화**: 장비 테이블에 방화벽 룰 버전 + 패키지 버전 통합 표시
...
**변경사항:**
- 기존: [방화벽 관리] [장비 관리] [배포 이력]
- 변경: [방화벽 관리] [패키지 관리] [장비 관리] [배포 이력]
- "패키지 관리" 탭 신규 추가 (패키지 업데이트용 + SSH 설정)

#### 3.3.2 패키지 관리 탭 (신규)

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ [방화벽 관리] [패키지 관리 X] [장비 관리] [배포 이력]                             │
├─────────────────────────────────────────────────────────────────────────────────────┤
│  패키지 관리                                                                       │
├─────────────────────────────────────────────────────────────────────────────────────┤
│  검색: [                    ] [찾기]                 [삭제]  [추가/수정]            │
...
**삭제 동작:**
1. 체크박스로 선택된 패키지이 없으면 아무 동작 안함
2. 선택된 패키지이 있으면 삭제 확인 다이얼로그 표시 ("선택한 N개 패키지을 삭제하시겠습니까?")
3. 확인 클릭 시 선택된 패키지 삭제 후 테이블 갱신
4. 취소 클릭 시 다이얼로그 닫기
...
| 배포선택 | 드롭다운 (방화벽 룰 배포 / 패키지 배포) |
| 배포 리스트 | 배포선택에 따라 룰 목록 또는 패키지 목록 표시 (단일 선택) |

**배포선택 옵션:**

| 옵션 | 설명 |
|------|------|
| 방화벽 룰 배포 | 방화벽 관리 탭에 등록된 룰 목록 표시 |
| 패키지 배포 | 패키지 관리 탭에 등록된 패키지 목록 표시 |
...
| 패키지 배포 | 패키지명 + 버전 (예: smartfw v2.1.0) | programs.json |
...
장비 추가/수정 다이얼로그가 표시된 상태에서 PPK `[찾아보기...]` 버튼을 클릭하면 파일 선택 다이얼로그가 중첩되어 열립니다. 패키지 관리 탭의 다이얼로그 중첩과 동일한 패턴 적용 필요.

#### 3.3.4 배포 이력 탭

기존 배포 이력 탭을 확장하여 방화벽 룰 배포와 패키지 업데이트 이력을 통합 관리합니다.
...
│  ☐  │ 2026-01-13 14:30:00 │ smartfw_2   │ 192.168.31.20:9200   │ 패키지     │ v3.5.1 │ 성공 │
│  ☐  │ 2026-01-12 10:00:00 │ firewall-03 │ 192.168.1.101        │ 방화벽 룰 셋 │ v1.0   │ 실패 │
│  ☐  │ 2026-01-11 09:00:00 │ firewall-04 │ 192.168.1.102        │ 패키지     │ v2.1.0 │ 실패 │
├─────────────────────────────────────────────────────────────────────────────────────┤
│                                << page 1/10 >>                                       │
└─────────────────────────────────────────────────────────────────────────────────────┘
```
...
기존 Firewall 모델을 확장하여 SSH 접속 정보와 패키지 버전 정보를 추가합니다.

```go
// ProcessInfo 설치된 패키지 정보
...
type Firewall struct {
    ...
    // 신규 필드 (패키지 버전) - 동적 목록
    Processes     []ProcessInfo `json:"processes"`    // 설치된 패키지 목록 (GET /device-report 응답과 동일 구조)
...
기존 DeployHistory 모델을 확장하여 방화벽 룰 배포와 패키지 업데이트 이력을 통합 관리합니다.
...
| 용어 | 설명 |
|------|------|
| smartfw | Smart Firewall - 방화벽 패키지 |
| smartway | Smart Gateway - 게이트웨이 패키지 |
