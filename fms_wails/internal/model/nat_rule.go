...
type NATRule struct {
	// 기본 필드
	NATType  NATType  `json:"natType"`  // DNAT, SNAT, MASQUERADE
	Protocol Protocol `json:"protocol"` // TCP, UDP, ANY

	// 매칭 조건
	MatchIP   string `json:"matchIP,omitempty"`   // 매칭할 IP (소스 또는 목적지)
	MatchPort string `json:"matchPort,omitempty"` // 매칭할 포트

	// 변환 대상
	TranslateIP   string `json:"translateIP,omitempty"`   // 변환할 IP
	TranslatePort string `json:"translatePort,omitempty"` // 변환할 포트

	// 인터페이스
	InInterface  string `json:"inInterface,omitempty"`  // 입력 인터페이스 (예: eth0)
	OutInterface string `json:"outInterface,omitempty"` // 출력 인터페이스 (예: eth1)

	// 추가 옵션
	Description string `json:"description,omitempty"` // 규칙 설명 (선택)
}
